...
namespace CarmeloSantana\WpOptions;
...
<?php
    ...
    public static function setDefaultOptions($version, $fields)
    {
        if (get_option(self::prefix('version')) === $version) {
            return;
        }

        foreach ($fields as $key => $option) {
            if (!get_option(self::prefix($key) and self::validateValue($option['default']))) {
                add_option(self::prefix($key), $option['default'], '', ($option['autoload'] ?? 'no'));
            }
        }

        update_option(self::prefix('version'), $version);
    }
